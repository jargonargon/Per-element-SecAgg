...
class SA_ServiceAgent(Agent):
    ...
    def __init__(self, id, name, type,
        ...
        self.accessThreshold = param.accessThreshold
        ...
        self.elapsed_time = {'REPORT': pd.Timedelta(0),
                             ...
                             'EXTRA_CHECK' : pd.Timedelta(0),
                             'EXTRA_RECON' : pd.Timedelta(0),
                             }
        ...
        self.recv_mi_cipher = {}
        self.recv_cipher_b_iut = {}
        self.recv_riut_shares = {}
        self.recv_recon_index = {}
        # Frequency for each index（uint16）
        self.index_counts = np.zeros(self.extraMask_len, dtype=np.uint16)
        # Contributing user list for each index
        self.user_list_per_index = np.empty(self.extraMask_len, dtype=object)
        # Default
        for i in range(self.extraMask_len):
            self.user_list_per_index[i] = []
        self.compressed_user_list_per_index = {}

        self.recv_emk_ut = {}
        self.recv_subgroup_id = {}
        ...
        self.recv_committee_shares_riut = {}
    ...
    def kernelStarting(self, startTime):
        ...
        self.kernel.custom_state['srv_extraCheck'] = pd.Timedelta(0)
        self.kernel.custom_state['srv_extraRecon'] = pd.Timedelta(0)

        ## Send communication size
        self.kernel.custom_state['srv_comm_label&cipher'] = 0
        self.kernel.custom_state['srv_comm_extraSend'] = 0
        self.kernel.custom_state['srv_comm_extra'] = 0

        ## Receive communication size
        self.kernel.custom_state['srv_comm_report'] = 0
        self.kernel.custom_state['srv_comm_recon1'] = 0
        self.kernel.custom_state['srv_comm_recon2'] = 0
    ...
    def kernelStopping(self):
        ...
        self.kernel.custom_state['srv_extraCheck'] += (
            self.elapsed_time['EXTRA_CHECK'] / self.no_of_iterations)
        self.kernel.custom_state['srv_extraRecon'] += (
            self.elapsed_time['EXTRA_RECON'] / self.no_of_iterations)
        ...
        self.kernel.custom_state['srv_comm_extra'] += self.message_size['EXTRA_SEND'] / self.no_of_iterations
        self.kernel.custom_state['srv_comm_recon2'] += self.message_size['EXTRA_RECV'] / self.no_of_iterations
    ...
    def receiveMessage(self, currentTime, msg):
        ...
        if msg.body['msg'] == "VECTOR":
            ...
            if msg.body['iteration'] == self.current_iteration:
                ...
                # parse the cipher for shares of riut
                self.recv_riut_shares[sender_id] = util.deserialize_tuples_bytes(msg.body['enc_riut_shares'])

                # parse the indices of ones vector
                self.recv_cipher_b_iut[sender_id] = msg.body['enc_b_iut']
        ...
        # Collect partial decryption results from decryptors
            ...
            if msg.body['iteration'] == self.current_iteration:
                ...
                self.cnt_decryptor_survive += 1
                if self.cnt_decryptor_survive <= 60:
                    ...
                    self.recv_emk_ut[sender_id] = msg.body['emk_ut']
                    self.recv_subgroup_id[sender_id] = msg.body['subgroup']
        ...
        elif msg.body['msg'] == "EXTRA_SHARED_RESULT":
                if msg.body['iteration'] == self.current_iteration:
                    self.recv_committee_shares_riut[sender_id] = util.deserialize_dim1_list(msg.body['shared_result_riut'])
                    # これを追加！：復号者インデックスの記録
                    self.recv_recon_index[msg.body['sender']] = msg.body['committee_member_idx']
    ...
    def report_read_from_pool(self):
        ...
        # assign user indices of ones to a new var. empty emmediately.
        self.cipher_b_iut = self.recv_cipher_b_iut
        self.recv_cipher_b_iut = {}

        # for each client, a list of encrypted riut shares
        self.riut_cipher = self.recv_riut_shares
        self.recv_riut_shares = {}
    ...
    def report_process(self):
        self.numCollectedVectors = len(self.user_vectors)
        self.agent_print("number of collected vectors:", self.numCollectedVectors)
        ...
        # b_iutの暗号文 (全clientから受信した全D個)
        self.df_b_iut_cipher = pd.DataFrame((self.cipher_b_iut).values())

        ## each row of df_riut_shares is the shares of an riut from a client
        self.df_riut_cipher = pd.DataFrame((self.riut_cipher).values())
    ...
    def report_send_message(self):
        ...
        ## Send both labels and ciphertexts at this time.
        cnt = 0
        committee_size = len(self.user_committee)
        for receiver_id in self.user_committee:
            receiver_index = cnt  # user_committee[cnt] と一致
            dec_target_riut = []
            ...
            temp_Message = Message({
                ...
                "b_iut_cipher": self.df_b_iut_cipher[cnt].tolist(),
            })

            self.sendMessage(receiver_id, temp_Message, tag="comm_dec_server")
    ...
    def reconstruction(self, currentTime):
        ...
        # another round.
        self.reconstruction_check()
        self.current_round = 4

        server_comp_delay = pd.Timestamp('now') - dt_protocol_start
        self.setWakeup(currentTime + server_comp_delay + param.wt_flamingo_reqExtraDec)
    ...
    def reconstruction_read_from_pool(self):
        ...
        self.emk_ut = self.recv_emk_ut
        self.recv_emk_ut = {}

        self.subgroup_id = self.recv_subgroup_id
        self.recv_subgroup_id = {}
    ...
    def reconstruction_process(self):
        ...
        if not self.dec_target_pairwise:
            ...
            """ Construct emk and derive the drop list of decryptors. """

            self.alive_decryptors_recon1 = {int(k) for k in self.emk_ut.keys()}
            emk_u = {}
            for id in self.alive_decryptors_recon1:
                my_subgroup = self.subgroup_id[id]
                if my_subgroup not in emk_u:
                    emk_u[my_subgroup] = self.emk_ut[id].copy()
                else:
                    emk_u[my_subgroup] += self.emk_ut[id]

            # subgroup をキー順（0,1,2,...）で並べる
            blocks = [emk_u[k] for k in sorted(emk_u.keys())]
            # 横方向に連結
            emk_concat = np.hstack(blocks)
            print(f"emk vector size: {emk_concat.shape}")

            print(f"alive decryptors: {self.alive_decryptors_recon1}")
            self.drop_decryptors_recon1 = {int(k) for k in self.user_committee} - self.alive_decryptors_recon1

            if len(self.drop_decryptors_recon1) == 0:
                print()
                print("###### No Dropped! ######")

            self.final_sum = (self.vec_sum_partial + cancel_vec + mi_vec 
                              - np.concatenate([[0]*(self.vector_len - self.extraMask_len), emk_concat]).astype(self.vector_dtype))
    
    def reconstruction_check(self):
        dt_protocol_start = pd.Timestamp('now')
        committee = list(self.user_committee)
        committee_size = len(committee)
        # self.df_riut_cipher.info()
        print()
        print(f"Assumed decrypt times: {len(self.drop_decryptors_recon1)*len(self.df_riut_cipher)}")
        print()

        for u in self.alive_decryptors_recon1:
            dec_target_riut = self.extract_ciphertexts_for_u(u)
            # print(np.array(dec_target_riut).shape)
            temp_Message = Message({"msg": "EXTRA",
                                    "iteration": self.current_iteration,
                                    "sender": self.id,
                                    "drop_decryptors_recon1": list(self.drop_decryptors_recon1),
                                    "dec_target_riut": util.serialize_tuples_bytes(dec_target_riut)
                                    })
    ...
    def extract_ciphertexts_for_u(self, u):
        committee = list(self.user_committee)
        committee_size = len(committee)
        receiver_index = committee.index(u)
        drop_set = set(self.drop_decryptors_recon1)

        result = []

        for sender_row in self.df_riut_cipher.itertuples(index=False):  # 各 client ごとに
            for v in drop_set:
                if v not in committee:
                    continue
                sender_index = committee.index(v)
                idx = sender_index * committee_size + receiver_index  # sender-major を前提
                ciphertext, nonce = sender_row[idx]
                result.append((ciphertext, nonce))

        return result  # 長さ = num_clients × num_drops 

    def extra_round(self, currentTime):
        print()
        print(f"The number of decryptors sends shares: {len(list(self.recv_committee_shares_riut.keys()))}")
        print()
        
        df_riut_shares = pd.DataFrame(self.recv_committee_shares_riut)  # shape: (num_clients * num_drops, num_committee)
        print(print(f"行数: {df_riut_shares.shape[0]}, 列数: {df_riut_shares.shape[1]}"))
        self.recv_committee_shares_riut = {}

        dt_protocol_start =  pd.Timestamp('now')

        # 閾値分に限定
        df_riut_shares = df_riut_shares.iloc[:, :param.SSThreshold]

        # ID順を取得
        committee_ids = df_riut_shares.columns
        recon_index_map = self.recv_recon_index

        num_clients = len(self.client_id_list)
        num_drops = len(self.drop_decryptors_recon1)

        # 再構築結果を格納する辞書
        reconstructed_rivt = {}  # {(client_id, drop_id) → int}

        for client_idx, client_id in enumerate(self.client_id_list):
            for drop_idx, drop_id in enumerate(self.drop_decryptors_recon1):
                # shareのインデックス = client_idx * num_drops + drop_idx
                share_row_index = client_idx * num_drops + drop_idx

                points = []
                for col in committee_ids:
                    share_value = df_riut_shares[col][share_row_index]
                    index = recon_index_map[col]
                    points.append((index, share_value))

                # 秘密復元
                secret_int, _ = points_to_secret_int(points=points, prime=self.prime, isecc=0)
                reconstructed_rivt[(client_id, drop_id)] = secret_int


        offset = self.vector_len - self.extraMask_len

        def get_subgroup_index(v):
            for idx, subgroup in enumerate(self.user_subgroups_committee):
                if v in subgroup:
                    return idx
            raise ValueError("self.id is not found in any subgroup")

        def estimate_block_sizes(N, k):
            base = N // k
            rem = N % k
            return [base + 1 if i < rem else base for i in range(k)]

        blocks_size = estimate_block_sizes(param.extraMask_len, param.numBlock)
        dropped_block_mask = {subgroup_id: np.zeros(blocks_size[subgroup_id], dtype=self.vector_dtype)
                            for subgroup_id in range(param.numBlock)}

        for uid in self.client_id_list:
            # 1) 各 drop した committee v について PRG を再構築
            sumvec_prg_committee = np.zeros(self.extraMask_len, dtype=self.vector_dtype)
            for v in self.drop_decryptors_recon1:
                subgroup_id = get_subgroup_index(v)

                rivt_int   = reconstructed_rivt[(uid, v)]
                rivt_bytes = rivt_int.to_bytes(self.key_length, 'big')

                # ハッシュ→シード→ChaCha20 でマスク生成
                mask_seed  = SHA256.new(data=rivt_bytes).digest()[:self.key_length]
                prg        = ChaCha20.new(key=mask_seed, nonce=param.nonce)
                prg_mask   = prg.encrypt(param.fixed_key * blocks_size[subgroup_id])
                mask_vec   = np.frombuffer(prg_mask, dtype=self.vector_dtype)

                dropped_block_mask[subgroup_id] += mask_vec

        # print(f"recontruct masks (dropped): {dropped_block_mask[0].shape, dropped_block_mask[1].shape, dropped_block_mask[2].shape}")
        # subgroup をキー順（0,1,2,...）で並べる
        dropped_blocks = [dropped_block_mask[k] for k in sorted(dropped_block_mask.keys())]
        dropped_mask_concat = np.hstack(dropped_blocks)
        self.final_sum -= - np.concatenate([[0]*(self.vector_len - self.extraMask_len), dropped_mask_concat]).astype(self.vector_dtype)

        self._user_rel_idx = {}

        self.agent_print("final sum:", self.final_sum[self.vector_len - self.extraMask_len:self.vector_len - self.extraMask_len+5])
        ...
        # Accumulate into time log.
        self.recordTime(dt_protocol_start, "EXTRA_RECON")
        self.alive_decryptors_recon1 = {}
        self.drop_decryptors_recon1 = {}
        
        print()
        print("######## Iteration completion ########")
        print(f"[Server] finished iteration {self.current_iteration} at {currentTime + server_comp_delay}")
        print()
        print(f"Surviving Clients: {self.numCollectedVectors}")
