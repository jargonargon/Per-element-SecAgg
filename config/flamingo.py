...
parser.add_argument('-r', '--record_result', type=bool, default=True, 
                    help='record result or not')
...
- wt_flamingo_reqExtraDec = pd.Timedelta('50s')
  - clients: 256, decryptor: 30, 
    - online clients: 245, online decryptor: 27
"""
...
########## Microbenchmarks Summary ##########
  Protocol Iterations  : {num_iterations}
  Number of Clients    : {num_clients}
  Number of Decryptors : {param.committee_size}
############ Experiment Setups ############
  Vector length       : {param.vector_len:,}
  Extra Mask Length   : {param.extraMask_len:,}
  Ratio of Ex/Vec     : {round((param.extraMask_len/param.vector_len) * 100)}%
  Vector type.        : {param.vector_type}
  Attack portion      : {param.numZeros:,}
  Access Threshold    : {param.accessThreshold}
  Secret Share t      : {param.SSThreshold}
  MaxDroplistSize     : {param.droplistSize}

  wt_reconstruction   : {param.wt_flamingo_reconstruction}
  wt_reqExtraDec      : {param.wt_flamingo_reqExtraDec}

  normal Sparsity     : {param.normalSparse}
  attackSparse.       : {param.attackSparse}

----- Service Agent (Mean Time per Iteration) -----
  - Report Step        : {results['srv_report']}
  - Crosscheck Step    : {results['srv_crosscheck']}
  - Reconstruction Step: {results['srv_reconstruction']}
  - Extra Reconstruct  : {results['srv_extraRecon']}
Total Time: {results['srv_report']+results['srv_crosscheck']+results['srv_reconstruction']+results['srv_extraRecon']}

----- Client Agent (Mean Time per Iteration) -----
  - Report Step        : {results['clt_report'] / num_clients}
  - Crosscheck Step    : {results['clt_crosscheck'] / param.committee_size}
  - Reconstruction Step: {results['clt_reconstruction'] / param.committee_size}
  - Extra Decryption   : {results['clt_extraDec'] / param.committee_size}
Total Time: {(results['clt_report']/num_clients) + (results['clt_crosscheck']/param.committee_size) + 
             (results['clt_reconstruction']/param.committee_size) + (results['clt_extraDec']//param.committee_size)}

----- Server Agent Communication (Average per Iteration) -----
  - Received from Clients (Report Step)       : {round(results['srv_comm_report']):,} Bytes
  - Sent to Decryptors (Labels & Ciphertexts) : {round(results['srv_comm_label&cipher']):,} Bytes
  - Received from Decryptors (RECON 1)        : {round(results['srv_comm_recon1']):,} Bytes
  - Sent to Decryptors (RECON 2)              : {round(results['srv_comm_extra']):,} Bytes
  - Received from Decryptors (RECON 2)        : {round(results['srv_comm_recon2']):,} Bytes
Total Send    : {round(results['srv_comm_label&cipher']+results['srv_comm_extra']):,} Bytes.
Total Receive : {round(results['srv_comm_report']+results['srv_comm_recon1']+results['srv_comm_recon2']):,} Bytes.

----- Client Agent Communication (Average per Client) -----
  - Receive from Server (Vector Download)     : {round((32/8)*param.vector_len):,} Bytes
  - Sent to Server (Report Step)              : {round(results['clt_comm_report'] / num_clients):,} Bytes
--- decryptor ---
  - Received from Server (Labels & Cipertexts): {round(results['dec_comm_label&cipher'] / param.committee_size):,} Bytes
  - Sent to Server (RECON 1)                  : {round(results['dec_comm_reconstruction'] / param.committee_size):,} Bytes
  - Received from Server (RECON 2)            : {round(results['dec_comm_extraRecv'] / param.committee_size):,} Bytes
  - Sent to Server (RECON 2)                  : {round(results['dec_comm_extraSend'] / param.committee_size):,} Bytes
Total Send    : {round((results['dec_comm_reconstruction']+results['dec_comm_extraSend'])/param.committee_size):,} Bytes.
Total Receive : {round((results['dec_comm_label&cipher']+results['dec_comm_extraRecv'])/param.committee_size):,} Bytes.
"""
...
# if args.record_result:
#   # 保存ファイル名（任意に変更可能）
#   filename = f"iter.{num_iterations}_.txt"
#   # 書き込み処理 
